...
Reprinted from the [competition data page](https://www.kaggle.com/c/how-much-did-it-rain-ii/data) the data columns are:
...
I performed a 10-fold cross-validation technique...
...
Summary of CART model in Appendix B
```{r cart_model_display}
mycart <- readRDS('./final_cart_model.rds')
prp(mycart)
```

Remember above values are the log -- need `exp(x) - 1` to remove log.

### GBM Model

Initially poor results

 Gaussian vs Laplace Distribution
 
#### GBM Grid Search with Cross Validation

Did a grid search with 10-fold cross validation.  Here are the results:
...
```{r mygbm_grid_plot_all, echo=FALSE, fig.cap="your caption"}
...
And zoom in to the bottom:

```{r gbm_grid_zoom, echo=FALSE, fig.cap="your caption"}
ggplot(mygbmgrid, aes(x = interaction.depth, y = MAE, group = trees_shrinkage)) + 
  geom_point(aes(color = shrinkage, size = n.trees)) + 
...
Replace below with final final gbm model:
...
Submitted two models at the end. 
...
Learned a lot.  Had I started this competition sooner, and had more time over holidays...

More variables -- did not use them all.  Use RhoHV to filter out bad values. 

GBM grids leveling out though -- probably little extra I could have gotten. 
